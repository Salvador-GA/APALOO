...
public class ListaEnlazadaDoble {
    ...
    /**
     * Constructor de la clase, inicializa la referencia inicio y fin a null y la
     * longitud a 0
     */
    ...
    public boolean insertar(String dato, int posicion) {
        if (posicion >= 0 && posicion <= longitud) {
            if (posicion == 0) {
                ...
                Nodo nuevo = new Nodo(dato, auxRecorrer.getSiguiente(), auxRecorrer);
    ...
    public boolean eliminar(int posicion) {
        if (posicion >= 0 && posicion < longitud) {
            if (posicion == 0) {
                ...
                auxRecorrer.getSiguiente().setAnterior(auxRecorrer);
                longitud--;
                return true;
            }
        } else {
    ...
    public String toStringInverso() {
        if (!esVacia()){
            String elementos = "[";
            Nodo auxRecorrer = fin;
            for (int i = 0; i < longitud - 1; i++) {
                elementos += auxRecorrer.getDato() + ", ";
                auxRecorrer = auxRecorrer.getAnterior();
            }
            elementos += auxRecorrer.getDato() + "]";
            return elementos;
        } else {
